...
function submitFeedback(data) {
    ...
    $("#feedback-submit").textContent = "Submitting...";
...
function hideWelcomeScreen() {
    ...
    // Restart 3D background for chat screen
    setTimeout(() => {
        stop3D();
...
function simulateAIResponse(userMessage) {
    ...
    setTimeout(() => {
        const responses = [
            "Thank you for your message! I'm processing your request about \"" + userMessage + "\".",
            "\n\nHere's what I can tell you:",
            "\n\nThis is a simulated response since the backend API isn't available in this static demo.",
        ...
        responses.forEach((part, index) => {
            setTimeout(() => {
                fullResponse += part;
                updateTypingIndicator(typingIndicator, fullResponse);

                if (index === responses.length - 1) {
                    completeResponse(typingIndicator, fullResponse);
                }
            }, index * 500);
        });
    }, 1000);
